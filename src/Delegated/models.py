...
def train(  # pylint: disable=too-many-arguments
    ...
    frac : float
        Parameter for the fraction of dataset available for training at current step.
    """
    ...
    for _ in range(epochs):
        net = _train_one_epoch(
            net, global_params, trainloader, device, criterion, optimizer, proximal_mu, frac
        )
...
def _train_one_epoch(  # pylint: disable=too-many-arguments
    ...
    for idx,(images, labels) in enumerate(trainloader):
        if idx<= int(frac*len(trainloader)):
...
def test(
    net: nn.Module, testloader: DataLoader, device: torch.device, frac:float = 1.0
) -> Tuple[float, float]:
    ...
    frac: float
        The fraction of dataset that the client has access at current timestep
    Returns
    -------
    Tuple[float, float]
    ...
    with torch.no_grad():
        for idx,(images, labels) in enumerate(testloader):
            if idx<= int(frac*len(testloader)):
                images, labels = images.to(device), labels.to(device)
                outputs = net(images)
